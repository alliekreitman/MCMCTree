...
for (i in 1:length(fasta_sequences)){
  sequences <- readDNAStringSet(fasta_sequences[i])
  
  # create output file paths that add _RT and _pol to the sample name
  output_rt <- gsub("\\.fasta$", "_RT.fasta", fasta_sequences[i])
  output_protease <- gsub("\\.fasta$", "_protease.fasta", fasta_sequences[i])
